...
const SPEED_NUBE: int = 55

var juego_activo: bool = false
var puertas_habilitadas: bool = false
var boton_acertijo_presionado: bool = false

@onready var dialogue_balloon: Node = $Dialogue
@onready var player: CharacterBody2D = $Player
# ❌ Eliminado: @onready var musica_fondo: AudioStreamPlayer2D = $MusicaFondo
...
func _ready() -> void:
	...
	# ❌ Eliminado: if musica_fondo: musica_fondo.stop()
...
func _iniciar_dialogo() -> void:
	var dialogue_resource: Resource = load("res://scenes/quests/story_quests/the_last_cards/3.The_House_of_Words/dialogues/room2.dialogue")
...
func _iniciar_juego() -> void:
	juego_activo = true
	# ❌ Eliminado: if musica_fondo and not musica_fondo.playing: musica_fondo.play()
...
func _game_over() -> void:
	juego_activo = false
	# ❌ Eliminado: if musica_fondo and musica_fondo.playing: musica_fondo.stop()
